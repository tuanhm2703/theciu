...
class VoucherPopupComponent extends Component
{
    ...
    public function loadVouchers() {
        if(customer()) {
            $this->vouchers = Voucher::public()->notExpired()->where('quantity', '>', 0)->with('voucher_type')->active()->saveable()->get();
            $saved_vouchers = customer()->saved_vouchers()->active()->notExpired()->get();
